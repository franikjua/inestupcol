...
import "./CertificationDonutChart.css";

const CertificationDonutChart = ({ semester }) => {
  ...
  useEffect(() => {
    ...
    fetch(`/data/${semester}/programs/programas.json`)
      ...
      .then((jsonData) => {
        // Filtrar por nivel y contar programas
        const levels = jsonData.reduce((acc, program) => {
          const nivel = program.nivel ? program.nivel.trim() : "Sin Nivel";
          acc[nivel] = (acc[nivel] || 0) + 1;
          return acc;
        }, {});

        // Convertir el objeto en un array de objetos para D3
        let formattedData = Object.keys(levels).map((nivel) => ({
          nivel,
          value: levels[nivel],
        }));
  ...
  useEffect(() => {
    ...
    const pie = d3
      .pie()
      .value((d) => d.value)
      .sort((a, b) => b.value - a.value); // Ordenar en el gráfico también
    ...
    svg
      ...
      .join("path")
      .attr("d", arc)
      .attr("fill", (d) => colorScale(d.data.nivel))
  ...
  return (
    <div className="chart-table-wrapper">
      <Title3>Distribución de Programas por Nivel</Title3>
      {loading ? (
        <p>Cargando datos...</p>
      ) : (
